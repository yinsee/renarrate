...
Common flags: `--force` (ignore cache), `--variant=final|johnwick|caveman|elon|trump|god|slj`, `--llm-model=<id>`, `--llm-url=http://localhost:11433/v1`, `--whisper-model=small.en`, `--language=en`.
...
- `POST /preview` takes `{text, variant}` JSON, runs `beautifyText` + `synthF5`, streams the resulting WAV back with the rewritten text in `X-Rewritten-Text` — lets the UI audition a voice/tone on a short snippet without uploading a video.
- `POST /jobs` takes a multipart upload (`file`, `variant`). The file is **hashed** (sha256, first 12 chars) and saved as `uploads/<hash>.<ext>`; re-uploading the same bytes skips the write. Returns `{jobId}`. `processFile` runs asynchronously and will short-circuit if `out/<hash>/<hash>.<variant.suffix>.mp4` already exists.
...
- `whisper.cpp`: the `whisper-cli` binary on `$PATH` (or via `WHISPER_CPP_CLI`) **and** a GGML model file — default `models/whisper-cpp/ggml-small.en.bin`, override with `WHISPER_CPP_MODEL` / `WHISPER_CPP_MODEL_DIR`.
...
- **Per-variant cache layout**: stages 1 and 2 (audio extraction + whisper transcription) cache under `out/<base>/` because they're variant-independent. Stages 3-6 (beautify, synth, SRT, mux) cache under `out/<base>/<variant>/`. The full-MP4 existence check at the top of `processFile` short-circuits all six stages. `--force` bypasses both layers.
