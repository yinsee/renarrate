...
// Extract a short speech segment from inputPath as a 24 kHz mono WAV suitable
// for use as an F5-TTS voice reference. Uses already-transcribed word timestamps
// to find the first targetDuration seconds of actual speech.
export async function extractVoiceRef({ ffmpeg, inputPath, words, outputWav, outputTxt, targetDuration = 12 }) {
  if ((await exists(outputWav)) && (await exists(outputTxt))) return;
  if (!words || words.length === 0) throw new Error('no transcribed words available for voice reference extraction');

  const speechStart = words[0].start;
  // Collect words that end within the target window (with a small buffer).
  const refWords = words.filter((w) => w.end <= speechStart + targetDuration + 0.5);
  const lastWord = refWords.length > 0 ? refWords[refWords.length - 1] : words[0];
  const duration = Math.min(lastWord.end, speechStart + targetDuration) - speechStart;

  // F5-TTS MLX requires 24 kHz; extract directly from the source video at that rate.
  await runTool(ffmpeg, [
    '-y', '-ss', speechStart.toFixed(3), '-t', duration.toFixed(3),
    '-i', inputPath,
    '-vn', '-ar', '24000', '-ac', '1', '-c:a', 'pcm_s16le',
    outputWav,
  ]);

  const refText = refWords.map((w) => w.text).join(' ').trim();
  await writeFile(outputTxt, refText, 'utf8');
}
