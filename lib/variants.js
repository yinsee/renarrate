...
const ORIGINAL_PROMPT = `You are polishing the voice-over for a product demo screen recording. The input is a raw spoken transcript full of natural disfluencies. Rewrite it as if the same speaker recorded it again at their best — confident, articulate, and completely free of hesitation.

**REWRITE GOALS:**
- Eliminate every disfluency: "um", "uh", "er", "like", "you know", "okay", "alright", "right", "so", "basically", "actually", "just", "kind of", "sort of", "I mean", false starts, self-corrections, and verbatim repetitions.
- Fix grammar and syntax for clean, natural spoken English.
- Keep sentences crisp and direct. Prefer active voice. No padding or throat-clearing.
- Preserve the speaker's authentic tone and emotional register — if they sound excited, keep that energy; if calm and methodical, keep that cadence.
- Keep the same first-person perspective ("I", "we") and conversational style the speaker uses; do not shift to third-person narration.
- Preserve the original meaning, action order, and level of technical detail exactly. Every step the viewer sees on screen must be described.
- Target roughly the same spoken length — do not summarize or expand.`;
...
export const VARIANTS = {
  ...
  original: {
    systemPrompt: ORIGINAL_PROMPT + COMMON_RULES,
    lengthScale: 1.0,
    suffix: 'original',
    chunkSize: 80,
    // Voice reference is extracted from the input video itself at runtime.
    dynamicVoice: true,
    voiceName: null,
  },
};
