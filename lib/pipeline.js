...
import { extractAudio, transcribe, extractVoiceRef, probeDuration } from './transcribe.js';
...
// Merge --variant (voice) and --style (narration) into a single effective variant.
// When --style is absent, the variant is used as-is.
// When --style is present: voice fields (voiceName, dynamicVoice) come from --variant;
// style fields (systemPrompt, chunkSize, validate, maxSentenceWords, lengthScale) come
// from --style; the suffix becomes "<variant>-<style>" to keep output paths distinct.
export function resolveVariant(opts) {
  const voice = getVariant(opts.variant);
  if (!opts.style) return voice;
  const style = getVariant(opts.style);
  return {
    systemPrompt: style.systemPrompt,
    chunkSize: style.chunkSize,
    validate: style.validate,
    maxSentenceWords: style.maxSentenceWords,
    lengthScale: style.lengthScale,
    voiceName: voice.voiceName,
    dynamicVoice: voice.dynamicVoice,
    suffix: `${voice.suffix}-${style.suffix}`,
  };
}
...
export async function processFile({ inputPath, opts, tools, onProgress }) {
  ...
  // For the 'original' variant, extract a voice reference segment from the
  // input video itself (first ~12s of speech at 24 kHz for F5-TTS MLX).
  let dynamicVoiceRef = null;
  if (variant.dynamicVoice) {
    const voiceRefWav = path.join(variantDir, 'voice_ref.wav');
    const voiceRefTxt = path.join(variantDir, 'voice_ref.txt');
    const refCached = (await exists(voiceRefWav)) && (await exists(voiceRefTxt));
    emit('extract', `[2b] extracting voice reference from input${refCached ? ' (cached)' : ''}`);
    await extractVoiceRef({
      ffmpeg: tools.ffmpeg,
      inputPath: abs,
      words,
      outputWav: voiceRefWav,
      outputTxt: voiceRefTxt,
    });
    const refText = (await readFile(voiceRefTxt, 'utf8')).trim();
    dynamicVoiceRef = { refAudio: path.resolve(voiceRefWav), refText };
  }
  ...
  const voiceLabel = variant.dynamicVoice ? 'source video' : variant.voiceName;
  emit('synth', `[4/6] synthesizing voice (f5 voice=${voiceLabel}, lengthScale=${variant.lengthScale})`);
  ...
  const { updatedSegments, totalVoiceDuration } = await synthesizeAndSchedule({
    ...
    voiceName: variant.dynamicVoice ? null : variant.voiceName,
