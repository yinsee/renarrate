...
export async function synthesizeAndSchedule({
  ...
  if (!voiceName && !voiceRef) throw new Error('voiceName or voiceRef is required (F5 voice reference)');
  await rm(segDir, { recursive: true, force: true });
  await mkdir(segDir, { recursive: true });

  const f5Ref = voiceRef || await loadVoiceRef(voiceRefDir, voiceName);
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (onSegment) onSegment(i, segments.length, duration, seg.text);
  }
