...
async function callLlm({ llmUrl, llmModel, systemPrompt, userMessage, temperature = 0.6 }) {
  let res;
  try {
...
// Prepended when the input came from a vision model (parse_video.py) rather than
// whisper. Without this nudge the LLM reads the input (already clean
// descriptive prose) as already satisfying COMMON_RULES rule 7/8 and passes it
// through with trivial edits. This directive EXPLICITLY overrides those rules
// for explain mode.
function explainPromptPrefix(explain) {
  if (!explain) return '';
  return `*** OVERRIDE: INPUT IS A FLAT VISUAL DESCRIPTION, NOT A TRANSCRIPT ***

The words below were NOT spoken by a human — they are a machine-generated, third-person, declarative description of what appears in each video frame. They already look like rule-7 "descriptive narration", so if you follow rule 8 ("keep length roughly similar") you will produce a useless near-copy. Do not do that.

HARD OVERRIDES for this task (they supersede COMMON_RULES 7 and 8):

  A. You MUST fully reimagine each sentence in your persona's voice. The scene, subjects, and chronology must be preserved; the WORDS and GRAMMAR must be entirely yours.
  B. Forbidden: copying any run of 4 or more consecutive words from the input into your output. Break the phrase up, replace nouns with persona-appropriate synonyms, restructure the clause.
  C. Rewritten length may differ significantly from the input — contract, expand, or fragment as your persona demands.
  D. Lead with voice, not fidelity. A viewer who never saw the input must be able to hear WHO is narrating from the first sentence.

Example — input: "A worker in a white suit reaches toward the moving sausages on the conveyor belt."
  BAD (passes rule 8, FAILS override B):  "A worker, dressed in a white suit, reaches toward the moving sausages on the conveyor belt."
  GOOD (caveman, override satisfied):     "White-suit man stretch hand. Want touch meat tube."
  GOOD (elon, override satisfied):         "And then, uh, the guy in the suit kind of reaches in, you know, toward — toward the meat, basically."
  GOOD (trump, override satisfied):        "Look at this guy — tremendous worker, tremendous — going right in, grabbing the sausage. Amazing."

Apply the persona described below with that same intensity.
...
async function beautifyChunk({ chunk, baseIndex, llmUrl, llmModel, systemPrompt, validate, maxSentenceWords, outputLanguage, explain = false }) {
  ...
  const effectiveSystemPrompt = explainPromptPrefix(explain) + languagePromptPrefix(outputLanguage) + systemPrompt;
