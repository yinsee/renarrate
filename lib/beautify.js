...
async function beautifyChunk({ chunk, baseIndex, llmUrl, llmModel, systemPrompt, validate, maxSentenceWords }) {
  const compact = chunk.map((w, i) => ({ i, text: w.text }));
  const mustKeep = extractMustKeepTokens(chunk);
  const lastIdx = chunk.length - 1;
  const baseNote = `Cover word indices 0 through ${lastIdx} inclusive. Every firstWord and lastWord must be between 0 and ${lastIdx}. First sentence's firstWord must be 0; last sentence's lastWord must be ${lastIdx}.`;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      const content = await callLlm({ llmUrl, llmModel, systemPrompt, userMessage });
      let parsed;
      try {
...
export async function beautifyText({ text, llmUrl, llmModel, systemPrompt }) {
