...
async function handleCreateJob(req, res) {
  ...
  const safeName = path.basename(file.filename || 'input.mov').replace(/[^\w.\-]/g, '_');
  const uploadPath = path.join(UPLOAD_DIR, `${jobId}_${safeName}`);
  ...
  const opts = { ...PIPELINE_DEFAULTS, variant, force: false };
