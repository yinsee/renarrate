...
import { PIPELINE_DEFAULTS, checkPrereqs, processFile, resolveVariant } from './pipeline.js';
...
async function handleCreateJob(req, res) {
  ...
  const style = parts.style ? parts.style.toString().trim() : null;
  const outputLanguage = (parts.outputLanguage || PIPELINE_DEFAULTS.outputLanguage).toString().trim();
  const boolField = (v) => {
    const s = (v || '').toString().trim().toLowerCase();
    return s === '1' || s === 'true' || s === 'on';
  };
  const explain = boolField(parts.explain);
  const slowVideo = boolField(parts.slowVideo);
  const parseNumField = (v, fallback, parser = parseFloat) => {
    if (v === undefined || v === null || v === '') return fallback;
    const n = parser(v.toString());
    return Number.isFinite(n) && n > 0 ? n : fallback;
  };
  const explainInterval = parseNumField(parts.explainInterval, PIPELINE_DEFAULTS.explainInterval, parseFloat);
  const explainMaxFrames = parseNumField(parts.explainMaxFrames, PIPELINE_DEFAULTS.explainMaxFrames, (s) => parseInt(s, 10));
  ...
  if (style && !VARIANTS[style]) return send(res, 400, { error: `unknown style: ${style}` });
  ...
  const opts = { ...PIPELINE_DEFAULTS, variant, style, outputLanguage, explain, explainInterval, explainMaxFrames, slowVideo, force: false };
...
async function handlePreview(req, res) {
  ...
  const styleName = body.style ? body.style.toString().trim() : null;
  ...
  let variant;
  try { variant = resolveVariant({ variant: variantName, style: styleName }); }
  catch (err) { return send(res, 400, { error: err.message }); }
  if (variant.dynamicVoice) {
    return send(res, 400, { error: 'preview is not supported for variant=original (no input video to extract voice from)' });
  }
